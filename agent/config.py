...
class Settings:
    ...
    min_net_edge: float = _f("MIN_NET_EDGE", 0.03)
    model_haircut: float = _f("MODEL_HAIRCUT", 0.02)
    kelly_fraction: float = _f("KELLY_FRACTION", 0.35)
    ...
    min_liquidity_usd: float = _f("MIN_LIQUIDITY_USD", 3000)
    min_volume_24h_usd: float = _f("MIN_VOLUME_24H_USD", 1000)
    min_book_multiple: float = _f("MIN_BOOK_MULTIPLE", 5)
    max_spread: float = _f("MAX_SPREAD", 0.08)
    loop_seconds: int = _i("LOOP_SECONDS", 180)
    estimate_batch: int = _i("ESTIMATE_BATCH", 10)
