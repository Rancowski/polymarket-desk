...
class Settings:
    ...
    max_position_pct: float = _f("MAX_POSITION_PCT", 0.06)
    min_net_edge: float = _f("MIN_NET_EDGE", 0.08)
    model_haircut: float = _f("MODEL_HAIRCUT", 0.05)
    kelly_fraction: float = _f("KELLY_FRACTION", 0.25)
    max_open_positions: int = _i("MAX_OPEN_POSITIONS", 10)
    max_category_pct: float = _f("MAX_CATEGORY_PCT", 0.30)
    daily_loss_halt_pct: float = _f("DAILY_LOSS_HALT_PCT", 0.04)
    weekly_loss_halt_pct: float = _f("WEEKLY_LOSS_HALT_PCT", 0.10)
    min_liquidity_usd: float = _f("MIN_LIQUIDITY_USD", 5000)
    min_volume_24h_usd: float = _f("MIN_VOLUME_24H_USD", 2000)
    min_book_multiple: float = _f("MIN_BOOK_MULTIPLE", 8)
    max_spread: float = _f("MAX_SPREAD", 0.06)
